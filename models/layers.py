...
### Added pre_trained layer, activation?
class Pre_trained(Layer):
    def __init__(self, units, w, b):
        super().__init__()
        #self.dense1 = Dense(units, activation = None)
        w0 = tf.constant_initializer(w)
        b0 = tf.constant_initializer(b)
        self.w = tf.Variable(initial_value=w0(shape = (682, units), dtype = tf.float32), trainable = False)
        self.b = tf.Variable(initial_value=b0(shape = (units, ), dtype = tf.float32), trainable = False)
        #self.dense2 = Dense(units, kernel_initializer= tf.constant_initializer(w), bias_initializer= tf.constant_initializer(b), activation=None)
